...
class ResourceHomeRepository{

    public function getAll(){
        try {
            $data_output = ResourceHomeModel::orderBy('updated_at', 'desc')->get();
            return $data_output;
        } catch (\Exception $e) {
            return $e;
        }
     ...
     public function addAll($request){
        try {
            $data =array();
            $dataOutput = new ResourceHomeModel();
            ...
            $finalOutput = ResourceHomeModel::find($last_insert_id); // Assuming $request directly contains the ID
    ...
    public function getById($id){
        try {
            $dataOutputByid = ResourceHomeModel::find($id);
            if ($dataOutputByid) {
    ...
    public function updateAll($request){
        try {
            $return_data = array();
            $dataOutput = ResourceHomeModel::find($request->id);
    ...
    public function updateOne($request){
        try {
            $updateOutput = ResourceHomeModel::find($request); // Assuming $request directly contains the ID
    ...
    public function deleteById($id){
            try {
                $deleteDataById = ResourceHomeModel::find($id);
                if ($deleteDataById) {

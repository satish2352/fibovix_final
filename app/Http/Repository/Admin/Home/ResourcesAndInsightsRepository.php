...
class ResourcesAndInsightsRepository  {
    ...
    public function getAll(){
     ...
     public function addAll($request){
        try {
            $data =array();
            $dataOutput = new ResourcesAndInsights();
            ...
            $finalOutput = ResourcesAndInsights::find($last_insert_id); // Assuming $request directly contains the ID
    ...
    public function getById($id){
        try {
            $dataOutputByid = ResourcesAndInsights::find($id);
            if ($dataOutputByid) {
    ...
    public function updateAll($request){
        try {
            $return_data = array();
            $dataOutput = ResourcesAndInsights::find($request->id);
    ...
    public function updateOne($request){
        try {
            $updateOutput = ResourcesAndInsights::find($request); // Assuming $request directly contains the ID
    ...
    public function deleteById($id){
            try {
                $deleteDataById = ResourcesAndInsights::find($id);
                if ($deleteDataById) {
                    if (file_exists_view(Config::get('DocumentConstant.RESOURCES_AND_INSIGHTS_DELETE') . $deleteDataById->image)){
                        removeImage(Config::get('DocumentConstant.RESOURCES_AND_INSIGHTS_DELETE') . $deleteDataById->image);
                    }
